...
/*
 ...
 TCS Digital Advanced Coding Code -

Please comment your answers in C, C++, Java, Python, Perl (these are the allowed languages)
...
public class WeightedMaze
{
    ...
    static void mazeRunner(Integer maze[][],int len,int i,int j,int wt)
    {
        if( wt < maze[i][j] ) wt=maze[i][j];
        
        if(wt>MaxWt) return;
        
        if(i<len-1) mazeRunner(maze,len,i+1,j,wt); //No down
        
        if(j<len-1) mazeRunner(maze,len,i,j+1,wt); //No right
        
        if(i==len-1 && j==len-1) 
        {
            if(wt<MaxWt) MaxWt=wt; //Better Solution
